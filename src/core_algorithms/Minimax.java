        ...
        if(game.isTerminal()){
            ...
            for(A move : game.getAllRemainingMoves()){
                ...
                if(a.score()>=beta){
                    ...
                    // won't allow this branch.
                    return new ScoreAndPath<>(a.score, null);
                }else if (a.score > alpha){
                    // Update best score and move sequence if this move is better
                    // for MAX player
                    alpha = a.score();
           ...
           return new ScoreAndPath<>(game.utility(), new ArrayList<>());
            ...
            for (A move : game.getAllRemainingMoves()) {
                ...
                if (b.score <= alpha){
                    ...
                    // MAX player won't allow this branch
                    return new ScoreAndPath<>(b.score, null);
                }else if (b.score()<beta){
                    // Update best score and move sequence if this move is better for MIN
                    beta = b.score();
